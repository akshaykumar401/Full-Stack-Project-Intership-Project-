...
import { RiHandHeartLine, RiCheckboxCircleFill, RiCloseCircleFill } from "@remixicon/react";
import { useState } from "react";
import { contactUs } from "../features/contact/contact.slice";
...
const Contect = () => {
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result.type === "contactUs/fulfilled") {
      setIsSubmit(true);
      setIsError(false);
      ...
      setEmail("");
      setMessage("");
      setSuccessErrorMessage("Your application has been submitted.");
      setIsLoading(false);
    } else {
      setIsSubmit(true);
      setIsError(true);
      setSuccessErrorMessage(result.payload || "Error submitting form. Please try again.");
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen bg-[#E5E2DB] pt-20 flex flex-col lg:flex-row items-center lg:items-stretch justify-center gap-10 px-6 lg:px-12 pb-12">
      ...
      <section className="w-full lg:w-[45%] bg-white p-8 lg:p-12 rounded-2xl border-2 border-gray-200 shadow-sm flex flex-col justify-center">
        ...
        <form className="mt-8 flex flex-col gap-6" onSubmit={handleFormSubmit}>
          ...
          >
            {isLoading ? <div className="flex items-center gap-2">Submitting...<div className="animate-spin rounded-full h-3 w-3 border-b-2 border-white"></div></div> : "Submit Application"}
          </button>
        </form>
      </section>
      ...
      {/* Notification Toast */}
      <div
        className={`fixed bottom-8 right-8 ${isError ? "bg-red-500" : "bg-[#2d6a6a]"} text-white px-6 py-4 rounded-xl shadow-2xl flex items-center gap-3 transform transition-all duration-500 z-50 ${isSubmit ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0 pointer-events-none"
          }`}
      >
        {isError ? (
        ...
        <div>
          <h4 className="font-bold text-lg leading-tight">{isError ? "Error!" : "Success!"}</h4>
          <p className="text-sm opacity-90 mt-0.5">{successErrorMessage}</p>
        </div>
      </div>
    </div>
  )
